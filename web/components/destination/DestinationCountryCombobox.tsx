...
import { isSupportedDestinationIso2 } from "@/lib/travelDestination";
...
export function DestinationCountryCombobox({
  countries,
  value,
  onChange,
  ...
  const filteredCountries = (countries ?? []).filter((c: any) => isSupportedDestinationIso2(c?.code ?? c?.iso2 ?? c?.id));
const [open, setOpen] = React.useState(false);

  const selected = React.useMemo(() => {
    if (!value) return null;
    return filteredCountries.find((c) => c.code === value) ?? null;
