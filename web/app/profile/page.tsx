...
export default function ProfilePage() {
  ...
  async function addPassport() {    const code = selectedCode;
    if (!code) return;
    setAdding(true);
    try {
      const res = await authFetch("/api/v1/passports", {
    ...
    <div className="space-y-8">
      ...
      <div className="rounded-2xl border bg-white p-5">
        ...
        {loading || passportsLoading ? (
          ...
          <p className="mt-4 text-sm text-red-700">{passportsError}</p>
        ) : passports.length === 0 ? (
              ...
              <li key={p.id} className="py-3 flex items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500">{p.countryCode}</p>
                  </div>
                </div>
