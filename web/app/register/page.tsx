      ...
      <div className="w-full max-w-md">
        ...
        <div className="overflow-hidden rounded-3xl border bg-white shadow-xl">
          ...
          <div className="bg-gradient-to-br from-indigo-600 via-violet-600 to-purple-700 px-8 py-8 text-white">
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white/20 text-2xl backdrop-blur">🚀</div>
            <h1 className="mt-4 text-2xl font-bold">{t.register.title}</h1>
            <p className="mt-1 text-indigo-200 text-sm">{t.register.subtitle}</p>
          </div>
          ...
          <div className="px-8 py-8">
            <form onSubmit={onSubmit} className="space-y-5">
              {/* Role selector */}
              <div>
                <label className="text-sm font-medium text-gray-700">{t.register.youAre}</label>
                <div className="mt-2 grid grid-cols-2 gap-3">
                  {[
                    { value: "STUDENT" as Role, icon: "🎓", title: t.register.studentTitle, desc: t.register.studentDesc, color: "border-indigo-500 bg-indigo-50", textColor: "text-indigo-700" },
                    { value: "UNIVERSITY" as Role, icon: "🏛️", title: t.register.univTitle, desc: t.register.univDesc, color: "border-violet-500 bg-violet-50", textColor: "text-violet-700" },
                  ].map((r) => (
                    <button key={r.value} type="button" onClick={() => setRoleState(r.value)}
                      className={`rounded-xl border-2 p-4 text-left transition ${role === r.value ? r.color : "border-gray-200 bg-white hover:border-gray-300"}`}>
                      <span className="text-xl">{r.icon}</span>
                      <p className={`mt-1.5 text-sm font-semibold ${role === r.value ? r.textColor : "text-gray-900"}`}>{r.title}</p>
            ...
            <p className="mt-6 text-center text-sm text-gray-500">
              ...
              <Link href="/login" className="font-semibold text-indigo-600 hover:underline">{t.register.loginLink}</Link>
            </p>
          </div>
        </div>
